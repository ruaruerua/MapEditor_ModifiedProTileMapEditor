...
public class ExporterRuntimeS : ScriptableObject
{
    ...
    private static string MeshToString(MeshFilter mf, Dictionary<string, OMATRuntime> materialList) 
    {
        ...
        for(int material=0; material < m.subMeshCount; material ++)
        {
            ...
            try
         		{
            		OMATRuntime objMaterial = new OMATRuntime();
   
            		objMaterial.name = mats[material].name;
                
                #if UNITY_EDITOR
            		if (mats[material].mainTexture)
            			objMaterial.textureName = UnityEditor.EditorUtility.GetAssetPath(mats[material].mainTexture);
            		else 
                #endif
    ...
    private static void Clear()
    {
   	...
   	private static void MaterialsToFile(Dictionary<string, OMATRuntime> materialList, string folder, string filename)
   	{
   		using (StreamWriter sw = new StreamWriter(folder + "/" + filename + ".mtl")) 
      {
       	foreach(KeyValuePair<string, OMATRuntime> kvp in materialList )
       	{
      		sw.Write("\n");
      		sw.Write("newmtl {0}\n", kvp.Key);
